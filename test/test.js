...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof zsumkbn, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( zsumkbn.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function computes the sum of all elements in a one-dimensional ndarray using an improved Kahan–Babuška algorithm', function test( t ) {
	var x;
	var v;

	x = new Complex128Array( [ 1.0, -2.0, -4.0, 5.0, 0.0, 3.0 ] );
	v = zsumkbn( [ vector( x, 3, 1, 0 ) ] );
	t.strictEqual( isSameComplex128( v, new Complex128( -3.0, 6.0 ) ), true, 'returns expected value' );

	x = new Complex128Array( [ -4.0, -5.0, -4.0, -5.0 ] );
	v = zsumkbn( [ vector( x, 2, 1, 0 ) ] );
